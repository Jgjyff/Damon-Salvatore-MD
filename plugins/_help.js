...
➮Fᴏᴜɴᴅᴇʀ- *AKASHI-SASAKI*
➮Oᴡɴᴇʀ - ${Config.ownername} 
➮Nᴜᴍ - ${owner.split(",")[0]} 
➮Mᴇᴍᴏ - ${formatp(os.totalmem() - os.freemem())} 
      \n *Damon-Salvatore-MD WHATSAPP BOT*\n\n ${readmore} 
╭──❰༆𝐀𝐥𝐥 𝐌𝐞𝐧𝐮༆❱ 
│🤖 Lɪꜱᴛ 
│🤖 Cᴀᴛᴇɢᴏʀʏ 
│🤖 Hᴇʟᴘ 
...
astro_patch.smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("*Damon-Salvatore-MD Ping!!!*");
...
astro_patch.cmd(
  ...
  async (message) => {
    try {
      message.reply(
        `*_Uptime of Damon-Salvatore-MD: ${runtime(process.uptime())}_*`
      );
    } catch (error) {
