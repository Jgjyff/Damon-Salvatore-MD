...
async function syncdb() {
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "Damon-Salvatore-MD") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "Damon-Salvatore-MD") => {
...
app.listen(randomPort, () =>
  console.log(
    "Damon-Salvatore-MD Server listening on http://localhost:" + randomPort + "/ "
  )
);
  ...
  if (isOfficialRepo) {
    ...
    if (
      ...
      try {
        ...
        log(
          "EMPTY SESSION_ID FROM Damon-Salvatore SERVER\nPLEASE SCAN THE QR AGAIN FROM [ " +
            global.scan +
            " ]\n\n\nERROR: ",
          error
        );
